...
#'@export
check.problem.set = function(ps.name,stud.path, stud.short.file, reset=FALSE, set.warning.1=TRUE, user.name="GUEST", do.check=interactive(), verbose=FALSE, catch.errors=TRUE, from.knitr=isTRUE(getOption('knitr.in.progress')) | !interactive(), use.null.device=FALSE, just.init=FALSE, stud.code=NULL) {
