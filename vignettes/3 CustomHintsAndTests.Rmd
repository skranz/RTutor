...
## Automatic Hints and Tests

I put a lot of effort to write sensible automatic tests and hints for RTutor. In most cases no customization should yield a descent experience for the user.

For example, consider a chunk were students shall write a dplyr pipe
...
dat %>%
  ...
  arrange(-mean_x)
```
The test fails with the not very informative message:
```
Error: 
...
Nevertheless, you there are cases were you want to customize your hints. Sometimes the automatic hints may reveal to much, sometimes they may reveal to little, or sometimes you just want to really polish your problem set and want hints with nicer text.
...
In general it is hard to predict where automatic hints will not be sufficient. If you use your problem sets in a course in which students submit there solution, take a look at the [README.md of the companion package RTutorSAGI](https://github.com/skranz/RTutorSAGI). It describes how you can analyse log files contained in the submission to detect and improve problematic tasks were many students got stuck. This allows for a nice iterative improvement process for your problem sets.
...
The code in the hint block will be evaluated in an environment in which all variables defined in earlier solved chunks are known. 

Also (at least if you use the default tests) the student's current code has been evaluated in the hint environment. 

This means whether the whether `z` exists in the hint environment depends on whether the user has defined it in her solution for the chunk or not. 

The function `true` is a robust version of `isTRUE` that never throws an error, but simply returns `FALSE` even if the expression cannot be evaluated, e.g. because `z` does not exist.

## Add custom hints to automatic hins with #< add_to_hint blocks
...
# Designing computations with many steps
