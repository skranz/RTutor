...
I tried my best to automatically test whether the student entered a correct solution or not. Yet, sometimes you may want to adapt the test parameters or written custom tests.
...
```
    ...
    ```
```
The test only passes if both checks in the test block pass.

The default test `check.assign` would only pass if the student would set `T=100`, but any `T >= 5` shall be considered correct. So we first replace the default test by a `check.variable` test that checks whether a numeric variable T of length 1 exists. Then we add another manual test `holds.true` that checks whether `T >= 5` holds.

Note that using the `holds.true` test without first checking that a numeric, variable T of length 1 exists can cause the `holds.true` test to crash.
