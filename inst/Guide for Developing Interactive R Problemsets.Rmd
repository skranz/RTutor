...
The parameter hint.name can specify a hint that will be shown to the user if the test fails and the user then types hint() in the R console.

#### #$ hints

In the code below the #$ hints command, you can specify several hints that can be shown if the student types hint() in the R console after a failed test.  In this section you specify hints using the add.hint function:
...
The first argument is a name of the hint, that can be used in the argument hint.name of a test to assign the hint to a specific test. (One hint can be assigned to multiple tests.)
...
test.H0.rejected(cor.test(p,eps),check.warning=FALSE, alpha.failure=0.05,
                 ...
                 success.message="Great, it looks as if p is indeed endogenous!")

# Check q
check.var(c("q"),
          exists=TRUE, length=TRUE, class=TRUE)

check.var("q", expr = beta0+beta1*p+eps,
      ...
      success.message = "Great, q looks correct!"
)
