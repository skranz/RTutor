...
The recommended way to create an interactive problem set is to write a solution file. It is an R file that contains as comments a description of the tasks as the student will see it, together with a solution to the problem set. In addition the solution can contain manual hints, tests or other commands like giving awards for a correct solution. Where no manual hints or tests are specified, RTutor will automatically generate tests and hints.
...
```{r eval=FALSE, tidy=FALSE}
  ...
  create.ps("example_ps_sol.r",ps.name="example_ps")
```
...
The create.ps command generates the files
  ...
  - example_ps.rmd: empty problem set in Rmarkdown format
  - example_ps_sample_solution.rmd: A sample solution of the problem set in Rmarkdown format
...
### #< test
Replace the default test, i.e. `check.call` or `check.assign` with a user specified test.
#### Example:
```{r}
#' Let u be a vector with 4 numbers
u = 1:4
#<
# Just check if u is numeric and has length 4 
check.var("u",1:4,exists=TRUE, length=TRUE, class=TRUE)
#>
...
### #< test.arg
Add additional arguments to the default tests `check.call` or `check.assign`. Simply specify the arguments as a comma separated list as illustrated in the example:
#### Example:
```{r}
#' Let u be a vector with 4 numbers
u = 1:4
...
### #<
Insert an additional test or give an award.
```{r}
beta0 = 100
#< test
  check.var("beta0",100,exists=TRUE, length=TRUE,class=TRUE)
#>
#<
  holds.true(beta0>0, failure.message="You must set beta0>0.")
#>
